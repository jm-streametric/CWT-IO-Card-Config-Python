...
def hexString(byteString: bytes, delimiter: str = " ") -> str:
    fullString = ""
    i = 0
    for byte in byteString:
        i += 1
        hexString = hex(byte).upper()[2:].rjust(2, "0")
        fullString += delimiter + hexString
        if i % 16 == 0:
            fullString += "\n"
    return fullString.removeprefix(delimiter)


def intString(byteString: bytes) -> str:
    fullString = ""
    i = 0
    for byte in byteString:
        i += 1
        intString = str(int(byte)).rjust(3)
